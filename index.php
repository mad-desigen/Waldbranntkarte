...
function fetchUrl(string $url): string {
    $ua = 'Mozilla/5.0 (compatible; WBI-GeoJSON/1.1)';
...
<div class="group"><label>Darstellung</label><label class="check"><input id="basemap" type="checkbox" checked> Grundkarte anzeigen</label><label class="check"><input id="heatmap" type="checkbox" checked> DWD-Heatmap anzeigen</label></div>
...
map.createPane('heatmapPane');map.getPane('heatmapPane').style.zIndex=650;
map.createPane('vectorPane');map.getPane('vectorPane').style.zIndex=700;
const baseLayer=L.tileLayer('https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png',{maxZoom:18,opacity:.55,pane:'tilePane'}).addTo(map);
...
function processedCanvas(im){const c=document.createElement('canvas'),ctx=c.getContext('2d',{willReadFrequently:true});const sx=Math.round(im.width*.015),sy=Math.round(im.height*.075),sw=Math.round(im.width*.965),sh=Math.round(im.height*.825);c.width=sw;c.height=sh;ctx.drawImage(im,sx,sy,sw,sh,0,0,sw,sh);const img=ctx.getImageData(0,0,sw,sh),p=img.data;for(let i=0;i<p.length;i+=4){const r=p[i],g=p[i+1],b=p[i+2];const max=Math.max(r,g,b),min=Math.min(r,g,b);if(r>242&&g>242&&b>242){p[i+3]=0}else if(max-min<18&&r>170){p[i+3]=0}else if(max-min<22&&r<170){p[i+3]=175}else{p[i+3]=245}}ctx.putImageData(img,0,0);return c}
function loadImage(src){$('loading').hidden=false;const im=new Image();im.onload=()=>{image=im;showImage(im);$('loading').hidden=true;status('DWD-Flächenkarte geladen. Die Heatmap liegt jetzt über der Grundkarte.');};im.onerror=()=>{$('loading').hidden=true;status('Bild konnte nicht geladen werden.');};im.src=src}
function showImage(im){const c=processedCanvas(im),url=c.toDataURL('image/png');if(overlay)map.removeLayer(overlay);overlay=L.imageOverlay(url,BOUNDS,{opacity:1,pane:'heatmapPane',interactive:false}).addTo(map);if(!$('heatmap').checked)map.removeLayer(overlay);map.fitBounds(BOUNDS)}
...
$('basemap').onchange=e=>{if(e.target.checked){baseLayer.addTo(map)}else{map.removeLayer(baseLayer)}};
$('heatmap').onchange=e=>{if(!overlay)return;if(e.target.checked){overlay.addTo(map)}else{map.removeLayer(overlay)}};
